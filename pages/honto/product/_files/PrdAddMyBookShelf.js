...
var prdAddMyBookShelf = {
	...
	add : function(pageBlockId, productId, haveProductId, displayType, ebookFlg) {
		if (!HC.isSubmitted) {
			...
			HC.Ajax.json(this.pageBlockId, this.complete, 'prdid=' + this.productId + '&having=on&havePrdId=' + this.haveProductId, false, 'dy_img_' +
						this.productId, prefix + this.haveProductId, this.error);
		}
	},
	 ...
	 * @param ebookFlg 電子書籍であることを表すフラグ
	 */
	_init : function(pageBlockId, productId, haveProductId, displayType, ebookFlg) {
	...
	complete : function(json) {
		...
		if (json.isRegistered === '1') {
			// 処理成功時、登録済みリンクに切替
			if (prdAddMyBookShelf.displayType === 'spList' || prdAddMyBookShelf.displayType === 'spDetail') {
				jQuery(prdAddMyBookShelf.selecter).replaceWith(prdAddMyBookShelf._createShelfLinkSp());
			} else if (prdAddMyBookShelf.displayType === 'pcList') {
				jQuery(prdAddMyBookShelf.selecter).replaceWith(prdAddMyBookShelf._createShelfLinkPcList());
			} else if (prdAddMyBookShelf.displayType === 'pcDetail' || prdAddMyBookShelf.displayType === 'pcDetailHbPrdList') {
				jQuery(prdAddMyBookShelf.selecter).replaceWith(prdAddMyBookShelf._createShelfLinkPcDetail());
				jQuery(prdAddMyBookShelf.selecterHbPrdList).replaceWith(prdAddMyBookShelf._createShelfLinkPcDetailHbPrdList());
			}
			// 後処理
			...
			if (prdAddMyBookShelf.displayType === 'pcDetailHbPrdList') {
				...
				jQuery(prdAddMyBookShelf.selecterHbPrdList).html(prdAddMyBookShelf.linkClone);
			} else {
	...
	error : function(xhr, textStatus, errorThrown) {
		...
		var linkUrl = window.location.pathname + '?prdid=' + prdAddMyBookShelf.productId + '&having=on&havePrdId=' + prdAddMyBookShelf.haveProductId;
	...
	_createShelfLinkPcList : function() {
		...
		var linkObj = jQuery('<li/>').append(jQuery('<a/>', {href : '/my/shelf.html'}).append('<span/>'));
		var linkText = 'My本棚を見る';

		linkObj.children().children().addClass('stBtn stUserAction stShelf stCurrent').text(linkText);
	...
	_createShelfLinkPcDetail : function() {
		...
		var linkObj = jQuery('<li/>').append(jQuery('<span/>').append(jQuery('<a/>', {href : '/my/shelf.html'})));
		var linkText = 'My本棚を見る';

		linkObj.children().addClass('stListLink08 stShelf stCurrent');
		linkObj.children().children().text(linkText);
	...
	_createShelfLinkPcDetailHbPrdList : function() {
		...
		return jQuery('<span/>').append(jQuery('<a/>', { href : linkUrl }).text(linkText));
	},
	...
	_createShelfLinkSp : function() {
		...
		var linkObj = jQuery('<p/>').append(jQuery('<a/>', {href : '/my/shelf.html'}));
		var linkText = 'My本棚を見る';
	...
	_term : function() {
		...
		if (jQuery('#dy_addWntBk_' + prdAddMyBookShelf.haveProductId).size()) {
			...
			if (prdAddMyBookShelf.displayType === 'spList') {
				jQuery('#dy_addWntBk_' + prdAddMyBookShelf.haveProductId).replaceWith(prdAddMyBookShelf._createWantButtonSpList());
			} else if (prdAddMyBookShelf.displayType === 'spDetail') {
				jQuery('#dy_addWntBk_' + prdAddMyBookShelf.haveProductId).replaceWith(prdAddMyBookShelf._createWantButtonSpDetail());
			} else if (prdAddMyBookShelf.displayType === 'pcList') {
				jQuery('#dy_addWntBk_' + prdAddMyBookShelf.haveProductId).replaceWith(prdAddMyBookShelf._createWantButtonPcList());
			} else if (prdAddMyBookShelf.displayType === 'pcDetail' || prdAddMyBookShelf.displayType === 'pcDetailHbPrdList') {
				jQuery('#dy_addWntBk_' + prdAddMyBookShelf.haveProductId).replaceWith(prdAddMyBookShelf._createWantButtonPcDetail());
				jQuery('#dy_addWntBk_hbPrdList_' + prdAddMyBookShelf.haveProductId).replaceWith(prdAddMyBookShelf._createWantButtonPcDetailHbPrdList());
			}
		}
		// ほしい本のエラーメッセージを除去
	...
	_clearMessage : function() {
		...
		if (jQuery('*[name=dy_wntBkMsg_' + prdAddMyBookShelf.haveProductId + ']').size()) {
			jQuery('*[name=dy_wntBkMsg_' + prdAddMyBookShelf.haveProductId + ']').remove();
		}
	},
	...
	_createWantButtonPcList : function() {
		...
		innerA.attr('href', 'javascript:prdWantBookAjax.add(' + '\'' + prdAddMyBookShelf.pageBlockId + '\',\'' + prdAddMyBookShelf.productId + '\',\'' + prdAddMyBookShelf.displayType + '\');');
	...
	_createWantButtonPcDetail : function() {
		...
		if (prdAddMyBookShelf.displayType === 'pcDetailHbPrdList') {
			...
			button.html(
					...
					+ 'id=dy_img_' + prdAddMyBookShelf.haveProductId + ' '
					+ 'onclick=prdWantBookAjax.add(' + '\'' + prdAddMyBookShelf.pageBlockId + '\',\'' + prdAddMyBookShelf.productId + '\',\'' + prdAddMyBookShelf.haveProductId + '\',\'' + 'pcDetail' + '\');'
					+ '>ほしい本に追加</span></a>');
		} else {
			// IE6,7,8でattrによるonclickイベントの追加が出来ない為、直接HTMLを編集
			button.html(
					...
					+ 'name=addWntBkBtnDisp '
					+ 'id=dy_img_' + prdAddMyBookShelf.haveProductId + ' '
					+ 'onclick=prdWantBookAjax.add(' + '\'' + prdAddMyBookShelf.pageBlockId + '\',\'' + prdAddMyBookShelf.productId + '\',\'' + prdAddMyBookShelf.haveProductId + '\',\'' + prdAddMyBookShelf.displayType + '\');'
					+ '>' + wantBookText + '</span></a>');
		}
		return button;
	},
	...
	_createWantButtonPcDetailHbPrdList : function() {
		...
		if (prdAddMyBookShelf.displayType === 'pcDetailHbPrdList') {
			...
			link.html(
					...
					+ 'id=dy_img_hbPrdList_' + prdAddMyBookShelf.haveProductId + ' '
					+ 'onclick=prdWantBookAjax.add(' + '\'' + prdAddMyBookShelf.pageBlockId + '\',\'' + prdAddMyBookShelf.productId + '\',\'' + prdAddMyBookShelf.haveProductId + '\',\'' + prdAddMyBookShelf.displayType + '\');return false;'
					+ '>'
					+ '欲しい本に追加する'
					+ '</a>');
		} else {
			...
			link.html(
					'<a ' + 'href=javascript:void(0); style="text-decoration: none;" '
					+ 'id=dy_img_hbPrdList_' + prdAddMyBookShelf.haveProductId + ' '
					+ 'onclick=prdWantBookAjax.add(' + '\'' + prdAddMyBookShelf.pageBlockId + '\',\'' + prdAddMyBookShelf.productId + '\',\'' + prdAddMyBookShelf.haveProductId + '\',\'' + 'pcDetailHbPrdList' + '\');return false;'
					+ '>'
					+ '欲しい本に追加する'
					+ '</a>');
		}
	...
	_createWantButtonSpList : function() {
		...
		inner.attr('onclick', 'prdWantBookAjax.add(' + '\'' + prdAddMyBookShelf.pageBlockId + '\',\'' + prdAddMyBookShelf.productId + '\',\'' + prdAddMyBookShelf.haveProductId + '\',\'sp\');');
