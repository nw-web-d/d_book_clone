...
var einsteinRecommendationsTrackPageView = {
    ...
    addTrackPageView : function (targetId, genreCode) {
        try {
            var trackPageViewArea = jQuery('#einstein-recommendations-track-page-view');
            if (typeof(adultauth) === 'undefined') {
                // アダルト認証BOXが表示されない場合（アダルト以外、または、アダルト認証済み）
                if (targetId != '') {
                    // 商品ID／シリーズIDが設定されている場合（商品詳細／シリーズ詳細の場合）
                    trackPageViewArea.append('<script>_etmc.push(["trackPageView", {"item" : "' + targetId + '"}]);</script>');
                }

                if (genreCode != '') {
                    // ジャンルコードが設定されている場合
                    trackPageViewArea.append('<script>_etmc.push(["trackPageView", {"category" : "' + genreCode + '"}]);</script>');
                }
