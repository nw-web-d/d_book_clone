...
var oneStepPurchase = {
    ...
    add : function(pageBlockId, formIdentifier, displayType) {
        ...
        if (!HC.isSubmitted) {
            ...
            // FWで必要なパラメータを設定
            oneStepForm.append('<input type="hidden" name="blockId" value="' + oneStepblockData.blockId + '" />');
            oneStepForm.append('<input type="hidden" name="className" value="' + oneStepblockData.className + '" />');
            oneStepForm.append('<input type="hidden" name="isPart" value="false" />');
            oneStepForm.append('<input type="hidden" name="noResponse" value="true" />');

            // EinsteinRecommendationsのtrackCart送信スクリプトを埋め込む
            try {
                var productId = oneStepForm.children('input[name="onestep-prdId"]').val();
                oneStepForm.append('<script>_etmc.push(["trackCart", {"cart" : [{"item" : "' + productId + '", "unique_id" : "' + productId + '"}]}]);</script>');
            } catch(e) {
