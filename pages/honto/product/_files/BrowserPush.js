...
jQuery.noConflict();
  ...
  var notificationPermission = {
    ...
    skipPermission : function() {
      // cookie設定
      document.cookie = 'notification_permission_skip=1; path=/; expires=' + this.getExpires() + '; max-age=' + this.getMaxAge();
...
var browserPushAjax = {
    ...
    pushPermission : function() {
        ...
        msglocal.requestPermission().then(function() {
            // 通知許諾ダイアログで許可が押された場合
            msglocal.getToken().then(function(curToken) {
    ...
    clearToken : function(alias) {
        ...
        msglocal.deleteToken(curToken).then(function() {
    ...
    getTokenFromStorage : function(alias) {
        ...
        msglocal.deleteToken(curToken).then(function() {
            msglocal.getToken().then(function(newToken) {
                ...
                // リクエストパラメータ作成
                var param = {fcmIdFromStorage : curToken, alias : alias, fcmIdToUser : newToken};
