...
var CidSetting = {
    ...
    settingData : null,
    date : new Date(),
    timestamp : function(){return this.date.getFullYear().toString()+(this.date.getMonth()+1).toString()+this.date.getDate().toString()+this.date.getHours().toString()}
}
...
jQuery(document).ready(function($) {
    ...
    $.extend(CidSetting, {
        ...
        setCid : function(pageBlockId, device) {
            ...
            if (!CidSetting.settingData) {
                ...
                setTimeout("CidSetting.setCid(" + pageBlockId + ", '" + device + "')", 1000);
            } else {
    ...
    function setCid(setting, pageBlockId) {
        if (setting.seq_class) {
            ...
            for (var index = 0; index < len; index++) {
                ...
                if (!linkObj.attr('class') || linkObj.attr('class').indexOf('dyNoCid') == -1) {
                    cidNum++;
                    // リンクにcidを追加
    ...
    function addCidToLink(obj, setting, index, lastIndex, cidNum) {
        ...
        obj.attr('href', link + paramText + 'cid=' + setting.cid_prefix + '_' + cidIndex);
        return;
    }
});
