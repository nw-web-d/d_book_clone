...
function checkSelectedPrdIdForSp(limit, pageBlockId, queryParam, prdId) {
	...
	if (prdId != null && prdId != '') {
		HC.Ajax.update(pageBlockId, 'errorArea', 'prdid[0]=' + prdId, null, 'errorArea');
...
function sendSiteCatalystSuggestData(slideInFlg) {
	...
	for(var i = suggestCount-1; i >= 0; i--) {
		if (DY.suggestList[i].className == "stCurrent") {
			...
			suggestWord = DY.suggestList[i].innerHTML.replace(/<[aA][^>]*>|<[eE][mM]>|<\/[eE][mM]>|<\/[aA]>/g,'');
		}
	}
