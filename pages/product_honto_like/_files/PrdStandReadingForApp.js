...
var appStandReadingAjax = {
     ...
     */
    standReadingWithLightBox : function(pageBlockId, productId, msuProductId, terminalType, deviceClass) {

        appStandReadingAjax.lightBoxFlag = true;

        appStandReadingAjax.standReading(pageBlockId, productId, msuProductId, terminalType, deviceClass);
    },
     ...
     * @param deviceClass  デバイスクラス(PC/SP)
     */
    standReading : function(pageBlockId, productId, msuProductId, terminalType, deviceClass) {
        ...
        if (!HC.isSubmitted) {
            ...
            if (appStandReadingAjax.deviceClass == 'PC' && appStandReadingAjax.terminalType == '00' && !appStandReadingAjax.lightBoxFlag) {
                // 非同期通信後に別画面を立ち上げるとブロックされる為、
                // 事前に別ブラウザを立ち上げてハンドルを退避しておく
                var style = HC.Window.Style.createObject(1024, 700, false, false, false, false, true, true, 0, 0);
                appStandReadingAjax.childWindow = HC.Window.open('about:blank', 'about:blank', style);
            }

            // サイトカタリストへ分析用データを送信
            appStandReadingAjax.sendSiteCatalyst(productId);

            HC.Ajax.jsonSync(pageBlockId
                         ,this.complete
                         ,'ebkPrdId=' + productId + '&msuPrdId=' + msuProductId + '&appStandReading=1'
                         ,false
                         ,'dy_img_' + productId
    ...
    executeViewerAppForPc : function(response) {
        ...
        if (appStandReadingAjax.isInnerBrowserAccess(response.osClass, response.innerBrowserFlag)) {
            ...
            if (appStandReadingAjax.deviceClass == 'PC' && appStandReadingAjax.terminalType == '00' && !appStandReadingAjax.lightBoxFlag) {
                if (response.stupVwrNm == 'TTPLG') {
                    ...
                    appStandReadingAjax.childWindow.location.href = response.downloadLink.split('\'')[1];
                } else {
