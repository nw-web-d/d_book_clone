...
var continueBuyApply = {
  ...
  _createLink: function(element) {
    ...
    if (this.orderSite == 'pc') {
      ...
      jQuery(element)
        .children()
        .attr('class', 'stBtn stSizeL stEbBtn stDisabled')
        .html(errorText)
      jQuery(element).after(
        '<p id="' +
          linkMsgId +
          '" class="stLink01 stMarginB15"><a href="/my/account/next-issue.html">続刊予約一覧で確認・キャンセル</a></p>'
      )
    }
    if (this.orderSite == 'smart_phone') {
      ...
      jQuery(element)
        .children()
        .attr('class', 'stBtn stSizeM')
        .prop('disabled', true)
        .html(errorText)
      jQuery(element).after(
        '<p id="' +
          linkMsgId +
          '" class="stLink01 stMarginB15 stCenter"><a href="/my/account/next-issue.html">続刊予約一覧で確認・キャンセル</a></p>'
      )
    }
  },
    ...
    if (message == 'already_registered') {
      ...
      if (this.orderSite == 'pc') {
        jQuery(element).before(
          '<p id="' +
            errorMsgId +
            '" class="stErrorMsg">すでにお申し込み済みの商品です。</p>'
        )
        var errorText = '申込み済み'
        jQuery(element)
          .children()
      ...
      if (this.orderSite == 'smart_phone') {
        jQuery(element).before(
          '<p id="' +
            errorMsgId +
            '" class="stErrorMsg"><strong>すでにお申し込み済みの商品です。</strong></p>'
        )
        var errorText = '申込み済み'
        jQuery(element)
          .children()
    ...
    if (message == 'now_maintenance') {
      if (this.orderSite == 'pc') {
        jQuery(element).before(
          '<p id="' +
            errorMsgId +
            '" class="stErrorMsg">現在、注文の受付を一時停止しております。</p>'
        )
      ...
      if (this.orderSite == 'smart_phone') {
        jQuery(element).before(
          '<p id="' +
            errorMsgId +
            '" class="stErrorMsg"><strong>現在、注文の受付を一時停止しております。</strong></p>'
        )
        var errorText = '申し込みできません'
        jQuery(element)
          .children()
    ...
    if (message == 'excluded') {
      if (this.orderSite == 'pc') {
        jQuery(element).before(
          '<p id="' +
            errorMsgId +
            '" class="stErrorMsg">続刊購入できない商品です。</p>'
        )
        var errorText = '申し込みできません'
        jQuery(element)
          .children()
      ...
      if (this.orderSite == 'smart_phone') {
        jQuery(element).before(
          '<p id="' +
            errorMsgId +
            '" class="stErrorMsg"><strong>続刊購入できない商品です。</strong></p>'
        )
        var errorText = '申し込みできません'
        jQuery(element)
          .children()
