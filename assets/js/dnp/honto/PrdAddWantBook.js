...
var prdWantBookAjax = {
      ...
      if (displayType === 'pcDetailHbPrdList') {
        HC.Ajax.json(
          ...
          'prdid=' +
            ...
            this.wantProductId,
          false,
          'dy_img_hbPrdList_' + this.productId,
          'dy_addWntBk_hbPrdList_' + this.wantProductId,
          this.error
        )
      } else {
        HC.Ajax.json(
          ...
          'dy_addWntBk_' + this.wantProductId,
          this.error
        )
      }
    }
  ...
  _clearMessage: function(productId) {
    ...
    if (
      jQuery('*[name=dy_wntBkMsg_' + prdWantBookAjax.wantProductId + ']').size()
    ) {
      jQuery(
        '*[name=dy_wntBkMsg_' + prdWantBookAjax.wantProductId + ']'
      ).remove()
    }
  },
  ...
  complete: function(json) {
    ...
    if (json.result.status === '1') {
      // 処理成功時、登録済みリンクに切替
      if (
        prdWantBookAjax.displayType === 'pcDetail' ||
        prdWantBookAjax.displayType === 'pcDetailHbPrdList'
      ) {
        jQuery(prdWantBookAjax.selecter).replaceWith(
          prdWantBookAjax._createLink()
        )
        jQuery(prdWantBookAjax.selecterHbPrdList).replaceWith(
          prdWantBookAjax._createLinkHbPrdList()
        )
      } else {
        jQuery(prdWantBookAjax.selecter)
          .children()
          .replaceWith(prdWantBookAjax._createLink())
      }
